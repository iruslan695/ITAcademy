...
public class JournalServiceImpl implements JournalService {
    ...
    public static String DOWNLOAD_PATH = ControllerConstant.UPLOAD_PATH;
    ...
    @Override
    public String isSubbed(String login, String journalName) throws ServiceException {
        DAOFactory daoFactory = DAOFactory.getInstance();
        JournalDAO journalDAO = daoFactory.getPublisherDAO();
        UserDAO userDAO = daoFactory.getUserDAO();
        ...
        try {
            int userId = userDAO.getUserID(login);
            int journalId = journalDAO.getTitleID(journalName);
            int pubId = journalDAO.getSubscriptionId(journalId, userId);
            if (pubId == 0) {
                result = false;
            } else {
            ...
            if (result) {
                stringResult = SQLCharacteristic.SUBBED;
            } else {
                stringResult = SQLCharacteristic.UNSUBBED;
            }
        } catch (DAOException e) {
            throw new ServiceException(e);
        }
    ...
    @Override
    public ArrayList<Integer> getJournalReleases(String journalName) throws ServiceException {
        DAOFactory daoFactory = DAOFactory.getInstance();
        JournalDAO journalDAO = daoFactory.getPublisherDAO();
        ArrayList<Integer> list;
        try {
            list = journalDAO.getJournalReleases(journalName);
        } catch (DAOException e) {
            throw new ServiceException(e);
        }
        return list;
    ...
    @Override
    public String getPublisherName(String journalName) throws ServiceException {
        DAOFactory daoFactory = DAOFactory.getInstance();
        JournalDAO journalDAO = daoFactory.getPublisherDAO();
        String result;
        try {
            result = journalDAO.getPublisherName(journalName);
        } catch (DAOException e) {
            throw new ServiceException(e);
        }
        return result;
